...
const itemListLdJson = {
  '@context': 'https://schema.org',
  '@type': 'ItemList',
  name: `Locksmith Services in ${BUSINESS.city}`,
  numberOfItems: serviceData.length,
  itemListElement: serviceData.map((s, i) => ({
    '@type': 'ListItem',
    position: i + 1,
    item: {
      '@type': 'Service',
  ...
  return (
    <section id="services" className="section-ref bg-[#F5F5F5]">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(itemListLdJson) }}
      />
      <div className="container-ref">
        ...
        <div className="text-center mb-12 sm:mb-16">
          ...
          <h2 className="text-[#17171A] max-w-4xl mx-auto px-2">
            Locksmith Services Across St. Louis County
          </h2>
          <p className="mt-4 text-[16px] sm:text-[18px] lg:text-[19px] text-gray-700 max-w-2xl mx-auto leading-relaxed px-4 font-medium">
            <strong className="text-[#17171A] font-bold">24/7 emergency response</strong> for lockouts, lock rekeys, key fob programming, and more, serving homes, cars, and businesses.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {featuredServices.map((service) => (
            <Link
              ...
              aria-label={`Learn more about ${service.name} in ${BUSINESS.city}`}
        ...
        {/* Related services prose — SEO keyword coverage in natural language */}
        <div className="mt-12 sm:mt-14 max-w-3xl mx-auto text-center px-4">
          <p className="text-[14px] sm:text-[15px] text-gray-600 leading-[1.7] normal-case">
            We also handle <strong className="font-semibold text-[#17171A]">front door lock installation</strong>, <strong className="font-semibold text-[#17171A]">smart lock setup</strong>, lock changes, keyless entry systems, key duplication, transponder key programming, car key cutting, and more. Whatever lock or key issue you're facing in <strong className="font-semibold text-[#17171A]">St. Louis County</strong>, our mobile team can handle it 24/7.
          </p>
        </div>

        {/* Bottom CTAs — primary button + secondary link */}
